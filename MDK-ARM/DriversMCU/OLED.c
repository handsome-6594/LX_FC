...
/*
移植相关内容：
step1：在cube中开启i2c口和dma通道，记得使能中断
step2：修改OLED_I2C为对应的i2c口
setp3：魔术棒 -> C/C++ -> misc Control 中加上 --no-multibyte-chars
step4：包含头文件，OLED_Init(), 调用函数，记得update
...
#define hdma_OLED_tx hdma_i2c1_tx
osSemaphoreId_t OLED_i2c_Binary_SemaphoreHandle = NULL;
...
void HAL_I2C_MemTxCpltCallback(I2C_HandleTypeDef *hi2c)
{
	if(hi2c ->Instance == I2C1)
	{
		...
		if(OLED_i2c_Binary_SemaphoreHandle != NULL)
		{
			xSemaphoreGiveFromISR(OLED_i2c_Binary_SemaphoreHandle, &highTaskWoken);
			if(highTaskWoken != pdFALSE)
			{
...
/*
发送命令的格式为：先寻址，后面的八位指定命令模式，再接上Data Byte（如果是命令，则为要写入命令的数据）
后面八位：
	第一位：control byte 直接给0，进入非连续模式
	第二位：D/C 需要写入命令，给0，如果需要写入数据，则需要给1
*/

static HAL_StatusTypeDef OLED_I2C_Send_Data(uint16_t MemAddress, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
	if(OLED_i2c_Binary_SemaphoreHandle != NULL)
	{
		HAL_I2C_Mem_Write_IT(&OLED_I2C,0x78, MemAddress, I2C_MEMADD_SIZE_8BIT, pData, Size);
		xSemaphoreTake(OLED_i2c_Binary_SemaphoreHandle, portMAX_DELAY);
	}
	else 
	{
		HAL_I2C_Mem_Write(&OLED_I2C,0x78, MemAddress, I2C_MEMADD_SIZE_8BIT, pData, Size, 0xffff);
	}
	return HAL_OK;
}
...
void OLED_WriteCommand(uint8_t Command)
{
	uint8_t wData[1];
	wData[0] = Command;
	OLED_I2C_Send_Data(0x00, wData, 1, 0xffff);
}


void OLED_WriteData(uint8_t *Data, uint8_t Length)
{
	OLED_I2C_Send_Data(0x40, Data, Length, 0xffff);
}
#endif
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	uint8_t wData = Command;
	j = HAL_SPI_Transmit(&hspi3, &wData, 1, HAL_MAX_DELAY);
	if(j != HAL_OK)
	{
		Error_Handler();
	}
...
void OLED_WriteData(uint8_t *Data, uint8_t Length)
{
	...
	i = HAL_SPI_Transmit(&hspi3, Data, Length, HAL_MAX_DELAY);
	if(i != HAL_OK)
	{
		Error_Handler();
	}
